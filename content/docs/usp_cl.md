...
Cross-dataset: `legislator_terms.state` + `district` matches `up_cdmaps.congressional_districts` for spatial district boundaries. Note that congressional_districts geometry is stored in EPSG:3857 (Web Mercator) while geocoded points across datasets are EPSG:4326 — apply a reprojection (e.g. `ST_Transform`) when joining geometries directly.
